...
interpolate <- function(path,path_write){
  ...
  for(i in 1:length(archive)){
    ...
    data$alfa <- -(log(data$AOD_500nm/data$AOD_675nm) )/log(500/675)
    # Calculation of AOD550
    data$aod_550 <- data$AOD_675nm*(500/675)**(-(data$alfa))
...
interpolate_s (path="D:/Josefina/papers_escritos/MAIAC_paper/datasets/aeronet/datasets",path_write="D:/Josefina/papers_escritos/MAIAC_paper/datasets/aeronet/datasets_interp_s/")
